...
def line_follow(frame):
    ...
    gray_frame = cv2.bilateralFilter(gray_frame, 15, 25, 25)
    # 二值化
    ...
    erode_frame = cv2.erode(255 - threshold_frame, kernel, iterations=1)
    # 膨胀
    kernel = np.ones((6, 6), np.uint8)
    dilate_frame = cv2.dilate(erode_frame, kernel, iterations=1)
    # 划定识别区域
    roi_frame = dilate_frame[250:300, 200:440]
    # cv2.imshow('line_roi', roi_frame)
    # 寻找轮廓
    contours, hierarchy = cv2.findContours(roi_frame, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    if len(contours) != 0:
        # 找到面积最大的轮廓
        ...
        if M['m00'] != 0:
            ...
            cy = int(M['m01'] / M['m00'])
            # 在原图中标记中心点
            cv2.circle(new_frame, (cx + 200, cy + 250), 5, (0, 0, 255), -1)
