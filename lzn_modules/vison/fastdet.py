...
class FastestDet():
    def __init__(self, classesPath, network, confThreshold=0.5, nmsThreshold=0.4 ):
        self.classes = list(map(lambda x: x.strip(), open(classesPath,
                                                          'r').readlines()))  ###这个是在coco数据集上训练的模型做opencv部署的，如果你在自己的数据集上训练出的模型做opencv部署，那么需要修改self.classes
        self.inpWidth = 352
        self.inpHeight = 352
        self.net = cv2.dnn.readNet(network)
